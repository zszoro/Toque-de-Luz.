...
const EMAIL_FROM = process.env.EMAIL_FROM || "Toque de Luz <onboarding@resend.dev>";
const VERIFICATIONS_FILE = path.join(DATA_DIR, "email-verifications.json");
...
async function redisSetJsonWithExpiry(key, value, seconds) {
...
export async function createUserWithPasswordHash({ name, email, phone, passwordHash }) {
    ...
    if (hasRedis()) {
        await redisSetJson(`user:${user.id}`, updatedUser);
    } else {
        const users = readCollection(USERS_FILE);
...
export async function saveEmailVerification({ name, email, phone, password, code }) {
    requireWritableStore();
    ...
    if (hasRedis()) {
        await redisSetJsonWithExpiry(`verify:${normalizedEmail}`, verification, 15 * 60);
        return verification;
    }

    const verifications = readCollection(VERIFICATIONS_FILE)
        .filter((item) => normalizeEmail(item.email) !== normalizedEmail);
    verifications.push(verification);
    writeCollection(VERIFICATIONS_FILE, verifications);
    return verification;
}
...
export async function getEmailVerification(email) {
    ...
    if (hasRedis()) {
        return redisGetJson(`verify:${normalizedEmail}`);
    }

    return readCollection(VERIFICATIONS_FILE).find((item) => normalizeEmail(item.email) === normalizedEmail) || null;
}
...
export async function deleteEmailVerification(email) {
    ...
    if (hasRedis()) {
        await redis(["DEL", `verify:${normalizedEmail}`]);
        return;
    }
...
export function isVerificationCodeValid(verification, code) {
    if (!verification) return false;
    return verification.codeHash === hashCode(String(code || "").trim());
}

export async function sendVerificationEmail(email, code) {
    if (!RESEND_API_KEY) {
        throw new Error("Envio de e-mail nao configurado. Adicione RESEND_API_KEY no Vercel e redeploy.");
    }
    ...
    const response = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
            ...
            subject: "Codigo de verificacao - Toque de Luz",
            html: `<p>Seu codigo de verificacao e:</p><h2>${code}</h2><p>Ele expira em 15 minutos.</p>`
        })
    });
    const data = await response.json().catch(() => ({}));

    if (!response.ok || data.error) {
        throw new Error(data.error?.message || "Nao foi possivel enviar o codigo para este e-mail.");
    }
